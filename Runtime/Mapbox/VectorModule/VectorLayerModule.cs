...
namespace Mapbox.VectorModule
{
	public class VectorLayerModule : LayerModule, ILayerModule
	{
		...
		{
			return _vectorModuleSettings.DataSettings.LayerRange.x <= targetZ && _vectorModuleSettings.DataSettings.LayerRange.y >= targetZ && _vectorSource.IsZinSupportedRange(targetZ);
		}
		...
		private void UpdateRetainedTiles(HashSet<CanonicalTileId> retainedTiles)
		{
			...
			foreach (var tileId in retainedTiles)
			{
				...
				{
					if(targetId.Z < _vectorModuleSettings.DataSettings.LayerRange.x)
						continue;
				...
				if (_vectorModuleSettings.LoadBackgroundData)
				{
					if (!_readyTiles.Contains(targetId))
					{
						for (int i = targetId.Z; i >= _vectorModuleSettings.DataSettings.LayerRange.x; i--)
						{
